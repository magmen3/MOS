...
hook.Add(
	...
	function()
		if game.SinglePlayer() then
			if CLIENT then
				Derma_Query(
					"You are running singleplayer server.",
					"It is recommended to play in multiplayer so some functions works better",
					"I understood",
					function()
...
hook.Add(
	...
	function()
		if ZBaseInstalled then
			ZBaseSetCategoryIcon("M.A.N.N. Offense Solutions - NPCs", "mos_icon_npcs32.png")
		end
	end
)
...
function MOS.CreatePM(name, pmodel, hmodel, manncorp)
	...
	if SERVER then
		MOS.Print("Loading playermodel: " .. MOSPMName)
	end
end
	...
	hook.Add(
		...
		function(cmd)
			...
			if (SwayMode:GetInt() == 1 and IsValid(Wep) and Wep.EnableSway) or SwayMode:GetInt() == 2 then
				if Wep ~= NULL and Wep.GetState and (Wep:GetState() == ArcCW.STATE_SIGHTS) then return end -- ArcCW compatibility
